...
- **Richer assignment attributes.** The *Upcoming work* sensor's `assignments`
  now include each item's `description` (HTML-stripped), and the *Missing work*
  sensor gains an `items` attribute listing each missing assignment as
  `{title, subject, due, description}` (with `due` normalized to an ISO date, to
  match the Upcoming sensor). Both are additive — the existing `assignments`
  shapes are unchanged — so cards, automations, and external renderers can show
  notes and per-item subject/due without extra calls.
